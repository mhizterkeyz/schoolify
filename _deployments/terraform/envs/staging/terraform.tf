...
resource "aws_ecr_repository" "enterprise-runner" {
  name = "enterpriseapi/staging/runner"
}
...
resource "aws_ecs_service" "staging" {
  ...
  load_balancer {
    target_group_arn = "${aws_lb_target_group.staging-1500.arn}"
...
resource "aws_lb_target_group" "staging-1500" {
  name        = "enterprise-staging-lb-tg"
  port        = 1500
  protocol    = "HTTP"
  target_type = "ip"
  vpc_id      = "${var.vpc_id}"
...
resource "aws_lb_listener_rule" "host_based_routing" {
  ...
  condition {
    host_header {
      values = ["deventerpriseapi.bento.africa"]
    }
  }
}
